...
POLICY_EVENTS = [
    {
        ...
        "time": "10:00",
        "name": "平台劳动者保障政策",
        "description": "出台平台劳动者保障政策，提高社保覆盖与工资结算透明度，加强平台用工监管。"
    }
]
...
# =========================================================
def infer_policy_effect(agent, policy_desc):
    profile_text = "\n".join([
        f"姓名：{agent.get('name', '')}",
        f"年龄：{agent.get('age', '')}",
        f"职业：{agent.get('job', '')}",
    ...
    prompt = f"""
你是城市生活模拟器的政策影响评估器。请基于政策描述与角色资料，推断该政策对角色状态的短期影响。
角色资料：
{profile_text}
政策描述：{policy_desc}
要求：
    ...
    for day in range(1, SIM_DAYS + 1):
        ...
        for time_str in timeline:
            ...
            for agent in agents:
                ...
                perc = perception(agent, time_str, social_context, policy_desc if policy else None)
                plan = planning(agent, perc)
                outcome = f"在【{activity}】中执行了【{act}】"
                refl = reflection(agent, outcome)

                if policy:
                    inferred = infer_policy_effect(agent, policy_desc)
