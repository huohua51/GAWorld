...
\node[item,below=11mm of skeleton,dashed] (life) {Life-events plugin\\working tree; not claimed};
\draw[flow] (registry) -- (policy);
\draw[flow] (registry) -- (skills);
\draw[hook] (registry) -- (life);
...
\node[font=\tiny,anchor=west,below=8mm of economy.west] {solid = connected/stable \quad dotted = hook dispatch \quad dashed = partial, skeleton, or unclaimed working-tree path};
\end{tikzpicture}
\end{document}
