...
Not all inputs use this bus.  Memory recall, world and social mechanisms, and
work integration retain direct paths within or around the pipeline, while the
economy predominantly uses hooks.  Nor does action selection currently imply
kernel-level validation: the Controller exposes a validator chain, but the
principal \texttt{select\_action} and execution path is not yet routed through
it.  Likewise, \texttt{record} writes the simulator's established artifacts
rather than making the kernel Recorder the single trace sink.  These boundaries
are central to interpreting the architecture: the lifecycle is connected and
configurable, but only partially migrated to kernel services.
