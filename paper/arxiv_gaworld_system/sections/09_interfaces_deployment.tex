...
\texttt{Controller}.  The latter is a partially integrated validation and
intervention abstraction with registered validators, named interventions, and
Recorder hooks.  The current Dashboard's primary run and mutation paths use its
own HTTP handlers and CLI subprocess management rather than routing all actions
through that Controller.  Describing the Dashboard as a complete front end for
the Controller would therefore overstate the microkernel migration.
