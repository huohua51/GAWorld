...
plugins.  The currently committed built-in plugin set covers interventions,
skills, interests, and life events.  Composability is deliberately not described as
unrestricted hot swapping: stages exchange a shared dictionary, dependencies
are only partly formalized, and memory, world, social, and work logic still
enter the main runtime through direct calls.  Economy integration is primarily
hook-based.  These compatibility paths are part of the documented system, not
...
Controller API for validators and named runtime interventions.  In the audited
runtime, however, the main action path does not yet route every selected
action through that Controller.  Accordingly, current controllability comes
mostly from configuration, scheduled events, hooks, and experiment scripts,
not from a universal action-governance layer.
