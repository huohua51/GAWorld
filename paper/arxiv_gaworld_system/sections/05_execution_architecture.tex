...
\begin{figure}[t]
  ...
  implementation surfaces; dashed paths denote a partial, skeleton, or
  unclaimed working-tree path.  The committed built-in plugins are
  Intervention, Skills, and Interests.  Memory, world, social, and work logic
...
Skills and Interests are also committed built-in plugins and use event
boundaries for their lifecycle contributions.  By contrast, the
\texttt{LifeEventsPlugin} visible in the audited working tree was not committed
in the frozen repository snapshot.  The paper therefore treats life events as
an implemented societal mechanism on its established path, not as a stable
plugin migration.  Similarly, no completed plugin claim is made for economy,
memory, world, social, or work.
...
Life events & Established mechanism; uncommitted plugin adapter in audit tree & Plugin migration unclaimed & Existing event path & Do not attribute archived runs to a stable life-event plugin \\
\bottomrule
\end{tabular}
