...
The microkernel migration is partial.  Intervention, Skills, Interests, and
Life Events have committed plugin integrations, while memory, world, social, and work
logic retain direct paths and economy is primarily hook-based.  The Controller
does not mediate every action, and the Recorder is not the sole provenance
stream.  Shared dictionaries and the context escape hatch remain useful for
compatibility but weaken schema enforcement and ownership boundaries.
...
artifacts easier to reconstruct.  Priorities are: (1) route the principal
action path through a typed Controller interface where validation is required;
(2) migrate remaining domain mechanisms incrementally behind declared
