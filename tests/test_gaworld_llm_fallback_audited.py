...
class TestFallbackChain(unittest.TestCase):
    ...
    def test_router_metadata_preserves_fallback_attempts(self):
        a = _StubProvider("a", fail=True)
        b = _StubProvider("b")
        router = _make_router({"default": "a", "fallback": ["b"]}, {"a": a, "b": b})

        result = router.call_with_metadata("hi")

        self.assertEqual("hello from b", result.text)
        self.assertEqual("b", result.metadata["selected_provider"])
        self.assertEqual(
            [False, True],
            [record["success"] for record in result.metadata["provider_calls"]],
        )
