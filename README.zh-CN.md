...
GAWorld 是一个面向城市社会行为实验与个人孪生网络的生成式多智能体项目。
它把人物画像、长期记忆、社会影响、环境扰动、政策事件、经济状态、地图移动、轻量平台干预评估、分布式社交 relay 和 LLM 决策过程组合到一个可回放、可对照、可扩展的模拟流程中。

## 项目概览

GAWorld 的目标不是简单地“跑一群 Agent”，而是提供一个可控制的社会实验场，同时支持本地运行的个人孪生。你可以：

- 让同一批智能体在不同事件或政策条件下运行
- 并行比较有事件和无事件的反事实场景
- 保留跨天记忆、习惯、意图和关系变化
- 让每个用户在本地设备上运行自己的个人分身，并只向中央 relay 共享公开社交摘要
- 在中央服务器上聚合多个本地分身，形成分布式的虚拟社交网络
- 针对单个 agent 或个人分身执行 What-if / 反事实推断
- 检查轨迹、日志、访谈结果和记忆文件
- 在不增加外部 API 的情况下评估 PolicySim 风格的推荐 / 曝光干预
- 通过本地 dashboard 修改配置、人物 profile、分布式社交状态并控制运行
...
- 本地优先的个人孪生配置：私有记忆留在本地，仅共享公开社交摘要
- 政策事件和环境事件模拟
- 个人 What-if / 反事实推演 CLI：针对单个分身比较“保持现状”和“注入假设”两条轨迹
...
- 可视化 trace 导出
- 单智能体采访 CLI
- 本地 dashboard：配置编辑、profile 编辑、运行控制、记忆查看、访谈、分布式社交快照
- 多机分布式 relay 通信模式，支持公开 profile、社交关系边、最近跨节点消息和 tick 同步
...
python generative_city_sim.py personal-what-if \
  --agent-id 2 \
  --question "如果我明天把主要精力放到面试准备而不是当前工作，会怎样？" \
  --sim-days 2
```
...
- 执行访谈
- 查看运行日志
- 查看分布式虚拟社交快照（连接中的分身、社交关系边、最近跨节点消息）
